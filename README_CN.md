...
这并不会消除所有元数据。网络时序、Cloudflare 账号层面的元数据和联系人关系线索仍然可能存在。TapChat 的目标是在保持可用性的前提下，降低中心化控制和跨用户元数据集中度。当前群协议的 Group Outbox 持有完整签名成员/角色 manifest，因此可以观察单群 roster。
...
- 当前”元数据隐私”不表示匿名通信，也不向承载该群的 Group Outbox provider 隐藏群成员关系。
- 每台设备维护一个一次性 MLS KeyPackage 池（目标大小 20，低于 5 个时补货），确保发起新会话消耗的是一个永远不会被派发给第二个联系人的 KeyPackage，从而将一次密钥泄露的影响限定在单次会话建立内。如果在补货完成前，短时间内有超过 20 个联系人同时向同一设备发起会话，超出部分会退化为一个长期存在的兜底 KeyPackage（约 70 天轮换一次）——这是当前设计中一个明确、范围有限的假设。
- 项目尚未经过第三方安全审计。
