...
impl CoreEngine {
    ...
    pub(super) fn send_text_message(
        ...
        self.maybe_rotate_direct_pcs(&conversation_id)?;
        self.merge_with_transport_flush(CoreOutput {
            state_update: CoreStateUpdate {
                messages_changed: true,
                conversations_changed: true,
    ...
    ///
    /// Deliberately does *not* decide whether to rotate. That decision is made
    /// on settled state — see [`Self::maybe_rotate_direct_pcs`].
    pub(super) fn observe_direct_application(&mut self, conversation_id: &str) {
        if !self.conversation_is_direct(conversation_id) {
            return;
        }
        if let Some(state) = self.state.conversations.get_mut(conversation_id) {
    ...
    /// and enqueue the commit. Returns whether a rotation happened.
    ///
    /// Unilateral by construction: create, merge and enqueue happen in one
    /// step, so healing never waits on the counterparty. `designated_committer`
    /// only decides who goes first (see `DirectPcsState::should_rotate`) and
    /// who wins a same-epoch collision (see [`Self::direct_pcs_arbitration`]).
    ///
    /// Must be called on settled state — never part-way through an inbound
    /// batch. A device returning from a long absence drains a backlog whose
    ...
    pub(super) fn maybe_rotate_direct_pcs(&mut self, conversation_id: &str) -> CoreResult<bool> {
        ...
        let member_device_ids = adapter.member_device_ids(conversation_id)?;
        ...
        let local_device_id = self
            ...
            .device_id
            .clone();
        let is_designated = designated_committer(&member_device_ids, epoch)? == local_device_id;
        let now_ms = current_unix_millis(self.state.message_nonce);
        let due = self
            .state
            .conversations
            .get(conversation_id)
            .is_some_and(|state| state.pcs.should_rotate(is_designated, now_ms));
        if !due {
            return Ok(false);
        }
        // Resolved before the merge so nothing fallible stands between merging
        // the commit and putting it in the outbox: a merge whose commit never
        // reaches the peer strands it forever, because the bytes are gone once
